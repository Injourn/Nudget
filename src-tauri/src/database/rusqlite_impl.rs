...
pub(crate) fn remove_account_sqlite(conn: &Connection, account: Account) -> anyhow::Result<()> {
    let result = remove_item(conn, DELETE_ACCOUNT, account.id);

    result
}

pub(crate) fn get_all_accounts_sqlite(conn: &Connection) -> anyhow::Result<Vec<Account>> {
    let result = get_all::<Account>(conn, GET_ALL_ACCOUNTS);

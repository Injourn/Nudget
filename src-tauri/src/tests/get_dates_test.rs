...
use crate::functions::get_dates_between::{get_dates_between_monthly, get_dates_between_weekly, get_succeeding_day_of_month, get_succeeding_day_of_week};
...
fn get_next_day_of_week_simple() {
    let date = 
    get_succeeding_day_of_week(NaiveDate::from_ymd_opt(2024, 9, 9).unwrap().borrow(),6);
...
fn get_next_day_of_week_next_week(){
    let date = 
    get_succeeding_day_of_week(NaiveDate::from_ymd_opt(2024, 9, 6).unwrap().borrow(),1);
...
fn get_next_day_of_week_equal() {
    let date =
    get_succeeding_day_of_week(NaiveDate::from_ymd_opt(2024, 9, 5).unwrap().borrow(),4);
